...
import { SearchOptionsDialog, SearchResultCard } from ".";

import type { SearchResult, SearchOptions, Folder } from "@/components/types";
...
const SearchSection: React.FC<SearchSectionProps> = ({ apiBaseUrl, authToken, onSearchSubmit }) => {
  ...
  const handleSearch = async () => {
    if (!searchQuery.trim()) {
      showAlert("Please enter a search query", { type: "error", duration: 3000 });
      return;
    }
